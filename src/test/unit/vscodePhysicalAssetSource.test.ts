...
describe("VS Code physical asset source exact resolution", function () {
  ...
  it("uses canonical open-document identity and falls back on incomplete filesystem evidence", () => {
    const script = [
      ...
      "let statMode = 'ready'; let metadataMode = 'missing'; let statCalls = 0; let metadataReads = 0; let abortOnStat; let invalidateOnStat; let directoryEntries = new Map(); let directoryReadCalls = 0; let textDocumentReads = 0;",
      ...
      "  readFile: async target => { if (!target.toString().endsWith('pack.mcmeta')) return Buffer.from('{}'); metadataReads++; if (metadataMode === 'missing') throw new FileSystemError('FileNotFound'); if (metadataMode === 'unavailable') throw new Error('provider offline'); if (metadataMode === 'invalid') return Buffer.alloc(0); if (metadataMode === 'overlay') return Buffer.from(JSON.stringify({ overlays: { entries: [{ directory: 'overlay', formats: { min_inclusive: 1, max_inclusive: 999 } }] } })); return Buffer.from('{}'); },",
