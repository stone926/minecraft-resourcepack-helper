...
describe("integrated RSGL ResourceUniverse navigation bridge", function () {
  ...
  it("returns scoped directory/ZIP/JAR/remote definitions and physical References without fake paths", () => {
    const script = [
      ...
      "    const first = { uri: 'file:///pack/assets/demo/models/block/consumer.json', origin: 'physical', range: { start: 16, end: 31 } };",
      "    return { context: { projectId: 'project' }, coverage: 'authoritative', locations: target.id === 'demo:block/multi' ? [first, { ...first, range: { start: 4, end: 10 } }, first] : [first] };",
      "  }",
      "};",
      "let generation = 0;",
      ...
      "  const openedBeforeMulti = opened.length;",
      "  const multiRefs = await resolveRsglResourceNavigation(facade, request('references','demo:block/multi','local'), new AbortController().signal);",
      "  assert.deepStrictEqual(multiRefs.locations.map(item => item.range), [{ start: { line: 1, character: 14 }, end: { line: 1, character: 29 } }, { start: { line: 1, character: 2 }, end: { line: 1, character: 8 } }]);",
      "  assert.strictEqual(opened.length - openedBeforeMulti, 1, 'one navigation batch opens a shared URI only once');",
      "  const abort = new AbortController(); abort.abort();",
