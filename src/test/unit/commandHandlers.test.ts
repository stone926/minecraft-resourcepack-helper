...
describe("contributed command handlers", function () {
  ...
  it("opens the configured vanilla assets folder and treats a missing setting as cancellation", () => {
    ...
    const script = [
      ...
      "let configured = null; const reads = []; const executions = [];",
      "const vscode = {",
      "  Uri: { file: fsPath => ({ scheme: 'file', fsPath, toString: () => `file:///${fsPath}` }) },",
      "  workspace: { getConfiguration: () => ({ get: key => { reads.push(key); return configured; } }) },",
      ...
      "configured = 'C:/Minecraft 资源/assets';",
      "assert.strictEqual(command(), undefined);",
      "assert.deepStrictEqual(reads, ['McResHelper.defaultMcAssetsPath', 'McResHelper.defaultMcAssetsPath']);",
      "assert.strictEqual(executions.length, 1);",
      "assert.strictEqual(executions[0][0], 'vscode.openFolder');",
      "assert.strictEqual(executions[0][1].fsPath, configured);",
