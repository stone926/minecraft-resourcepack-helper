...
interface BuildModule {
  createBuildPlan(
    targetName: string,
    options?: { mode?: "build" | "typecheck" | "bundle" }
  ): readonly NodeStep[];
  executeBuildPlan(
  ...
  parseBuildArguments(args: string[]): { targetName: string; mode: string };
}
...
describe("build orchestration", () => {
  ...
  it("plans focused typecheck and bundle work without npm recursion", () => {
    assert.deepStrictEqual(stepCommands(build.createBuildPlan("main")), [
      ...
      ["scripts/build-bundles.mjs", "main"]
    ]);
    assert.deepStrictEqual(stepCommands(build.createBuildPlan("rsgl")), [
      ...
      ["scripts/build-bundles.mjs", "rsgl"]
    ]);
